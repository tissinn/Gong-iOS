...
class GameScene: SKScene {
    ...
    override func sceneDidLoad() {
		...
		playerGoal.position = CGPoint(x: 0, y: -(frame.size.height / 2) - playerGoal.size.height / 2)
		addChild(playerGoal!)
		...
		player.position = CGPoint(x: 0, y: -(frame.size.height / 2) + 100)
		...
		enemy.position = CGPoint(x: 0, y: (frame.size.height / 2) - 100)
